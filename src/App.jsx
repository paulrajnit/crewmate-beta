...
const dash=rego.toUpperCase(); // e.g. VH-VFH

// Try multiple ADS-B sources in order
const sources=[
// Source 1: adsb.lol (no key needed)
()=>fetchWithTimeout(`https://api.adsb.lol/v2/registration/${clean}`,6000),
// Source 2: OpenSky Network (public, no key)
()=>fetchWithTimeout(`https://opensky-network.org/api/states/all?time=0&icao24=all`,5000)
.then(d=>{
if(!d?.states)return null;
const s=d.states.find(r=>r[1]&&r[1].trim()===clean);
if(!s)return null;
return{ac:[{hex:s[0],r:dash,t:“A320”,flight:s[1]?.trim()||””,
alt_baro:s[7]?s[7]*3.281:0,gs:s[9]||0,on_ground:s[8]}]};
}).catch(()=>null),
];
...
try{
const r=await src();
if(r?.ac?.length>0){data=r;break;}
}catch(e){}
}

if(!data?.ac?.length){
// Return static fallback with known VH-VFH data if all APIs fail
if(clean===“VHVFH”||dash===“VH-VFH”){
return{
rego:“VH-VFH”,type:“A320-232”,operator:“Jetstar Airways”,
msn:“5211”,built:“2012”,age:“14yr”,
engines:“2× CFM56-5B4”,seats:“180”,
status:“On Ground”,flight:””,alt:“ground”,gs:0,
fetchedAt:new Date().toLocaleTimeString(“en-NZ”,{hour:“2-digit”,minute:“2-digit”}),
history:[],historyDate:“Live data unavailable — check FlightAware or FR24”,
...
// Fetch history via hex
let history=[];
if(ac.hex){
const histSources=[
()=>fetchWithTimeout(`https://api.adsb.lol/v2/icao/${ac.hex}/recent`,5000),
];
for(const hs of histSources){
try{
const hist=await hs();
if(hist?.ac?.length>0){
...
const src=data===sources[0]?“adsb.lol”:“ADS-B”;
return{…parsed,hex:ac.hex,history,
historyDate:`Today · via ${src}`,
source:src};
}
    ...
    <div style={{animation:"fadein .5s ease"}}>
      ...
      <div style={{marginBottom:12}}>
        <span style={{
          ...
          fontSize:"12px",transform:"scaleX(1.55)",transformOrigin:"left center",
  ...
  {step===2&&(
    <div style={{animation:"fadein .4s ease"}}>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:16}}>
        {[
          {k:"rate",   label:"Pay Rate",    sub:"/hr", placeholder:"e.g. 68.50"},
          {k:"sbRate", label:"Standby Rate",sub:"/hr", placeholder:"e.g. 34.25"},
        ].map(({k,label,sub,placeholder})=>(
          <div key={k}>
            <div style={{height:28,display:"flex",alignItems:"center",marginBottom:6}}>
              <div style={{fontSize:10,color:C.muted,letterSpacing:1.5,lineHeight:1}}>
                {label.toUpperCase()}
                <span style={{color:C.muted,opacity:.6}}> {sub}</span>
              </div>
            </div>
        ...
        <div>
          ...
          <div style={{fontSize:18,fontWeight:800}}>
            {page==="menu"?"Settings":page==="nickname"?"Edit Nickname":
             page==="profile"?"My Profile":page==="codes"?"Duty Codes":
             page==="siri"?"Siri Shortcut":"Privacy & Security"}
          </div>
        </div>
      </div>
      ...
      {page==="menu"&&(
        <div>
          ...
          <div style={{background:`linear-gradient(135deg,${C.accent}18,${C.blue}12)`,
            ...
            <div style={{flex:1}}>
              <div style={{display:"flex",alignItems:"center",gap:8}}>
                ...
                <button onClick={()=>setPage("nickname")}
                  style={{background:"none",border:"none",cursor:"pointer",
                    padding:"2px 6px",borderRadius:6,
                    color:C.muted,fontSize:14,lineHeight:1}}>✏️</button>
              </div>
          ...
          {[
            ...
            {icon:"siri", label:"Siri Shortcut", sub:"Add one-tap roster update to iPhone",page:"siri"},
              ...
              <div style={{fontSize:22,width:36,height:36,borderRadius:10,
                ...
                {item.icon==="siri"
                  ? <svg width="22" height="22" viewBox="0 0 22 22" fill="none">
                      ...
                      <path d="M3 11 Q5.5 5 8 11 Q10.5 17 13 11 Q15.5 5 18 11" 
                        stroke="url(#sg)" strokeWidth="2.2" strokeLinecap="round" 
                        strokeLinejoin="round" fill="none"/>
                      <path d="M1 11 Q2 8.5 3 11" 
                        stroke="url(#sg)" strokeWidth="1.5" strokeLinecap="round" fill="none" opacity="0.5"/>
                      <path d="M19 11 Q20 13.5 21 11" 
                        stroke="url(#sg)" strokeWidth="1.5" strokeLinecap="round" fill="none" opacity="0.5"/>
                    </svg>
                  : item.icon}
              </div>
      ...
      {page==="privacy"&&(
        <div>
          ...
          <button onClick={()=>{
            ...
            border:`1px solid ${C.red}40`,borderRadius:12,
            padding:"12px 0",color:C.red,fontSize:12,
            fontWeight:600,cursor:"pointer",letterSpacing:.5}}>
            🗑 Clear All Data &amp; Restart Onboarding
          </button>
        </div>
      )}
...
s.textContent=`@keyframes cm-slide-left{from{opacity:0;transform:translateX(44px)}to{opacity:1;transform:translateX(0)}} @keyframes cm-slide-right{from{opacity:0;transform:translateX(-44px)}to{opacity:1;transform:translateX(0)}} @keyframes cm-slide-up{from{opacity:0;transform:translateY(32px)}to{opacity:1;transform:translateY(0)}} @keyframes cm-sheet-up{from{transform:translateY(100%)}to{transform:translateY(0)}} .cm-tab-left{animation:cm-slide-left .22s cubic-bezier(.25,.46,.45,.94) both} .cm-tab-right{animation:cm-slide-right .22s cubic-bezier(.25,.46,.45,.94) both} .cm-slide-up{animation:cm-slide-up .25s cubic-bezier(.25,.46,.45,.94) both} .cm-sheet{animation:cm-sheet-up .3s cubic-bezier(.25,.46,.45,.94) both} .cm-btn{transition:transform .12s ease,opacity .12s ease;-webkit-tap-highlight-color:transparent;cursor:pointer} .cm-btn:active{transform:scale(.93);opacity:.75} .cm-card{transition:transform .12s ease,box-shadow .12s ease} .cm-card:active{transform:scale(.98)} @keyframes cm-blink{0%,100%{opacity:1}50%{opacity:.4}} .cm-current-sector{animation:cm-blink 2s ease-in-out infinite} @import url('https://fonts.cdnfonts.com/css/horizon'); .horizon-date{ font-family:'Horizon','Barlow Condensed','Arial Narrow',sans-serif; font-weight:800; text-transform:uppercase; letter-spacing:1px; font-size:10px; line-height:1; background:linear-gradient(90deg,#FF6B35 0%,#FFB347 55%,#FF8C35 100%); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; display:block; padding:6px 0 10px 0; user-select:none; transform:scaleX(1.55); transform-origin:left center; }`;
document.head.appendChild(s);
},[]);
}

// ── Animated background planes — pure CSS keyframes, no re-renders ────────
const PLANE_DEFS = [
{id:“p1”, dur:42, delay:0,  color:”#FF5500”, glow:”#FF6B35”, op:0.55, dir:1,  yStart:8,  dy:18,  size:17},
{id:“p2”, dur:58, delay:16, color:”#00EEFF”, glow:”#00C8FF”, op:0.45, dir:-1, yStart:25, dy:-14, size:20},
{id:“p3”, dur:46, delay:30, color:”#FF5500”, glow:”#FF6B35”, op:0.50, dir:1,  yStart:60, dy:-20, size:14},
{id:“p4”, dur:64, delay:8,  color:”#00EEFF”, glow:”#00C8FF”, op:0.40, dir:-1, yStart:75, dy:16,  size:18},
{id:“p5”, dur:38, delay:24, color:”#FF5500”, glow:”#FF6B35”, op:0.50, dir:1,  yStart:88, dy:-12, size:15},
];
function PlaneBg(){
return(
<div style={{position:“fixed”,inset:0,pointerEvents:“none”,zIndex:0,overflow:“hidden”}}>
<style>{`@keyframes flyR{from{left:-8%}to{left:108%}} @keyframes flyL{from{left:108%}to{left:-8%}} @keyframes driftD{from{margin-top:0}to{margin-top:20vh}} @keyframes driftU{from{margin-top:0}to{margin-top:-20vh}}`}</style>
{PLANE_DEFS.map(p=>{
// SVG nose points UP (0°). Right travel = +90°, Left = -90°. Diagonal ±15°.
const base     = p.dir===1 ? 90 : -90;
const tilt     = (p.dy>0 ? 15 : -15) * (p.dir===1 ? 1 : -1);
const angleDeg = base + tilt;
const xAnim     = p.dir===1 ? “flyR” : “flyL”;
const yAnim     = p.dy>0  ? “driftD” : “driftU”;
return(
<div key={p.id} style={{
position:“absolute”,
top:`${p.yStart}%`,
...
animation:`${xAnim} ${p.dur}s ${p.delay}s linear infinite, ${yAnim} ${p.dur}s ${p.delay}s linear infinite`,
}}>

```
        <svg width={p.size} height={p.size} viewBox="0 0 24 24" fill="none"
          style={{display:"block",transform:`rotate(${angleDeg}deg)`,
            filter:`drop-shadow(0 0 3px ${p.glow}) drop-shadow(0 0 6px ${p.glow}50)`}}>
...
rgdCount:0,phoCount:0,groundPaidHours:0,rosterMonth:4,rosterYear:2026};
});
...
// ── GridView: compact calendar-style roster ──────────────────────────────
function GridView({allDays, onSelect}){
const [expanded, setExpanded]=useState(null);
return(
<div>
{Object.entries(allDays).map(([date,entry])=>{
const isDuty   = entry.type===“duty”;
const flights  = isDuty ? [] : (entry.flights||[]);
const meta     = isDuty ? (DUTY_META[entry.code]||{color:C.muted,icon:“📅”,label:entry.code}) : null;
const totalH   = flights.reduce((s,f)=>s+(f.dutyHours||0),0);
const hasOv    = flights.some(f=>f.overnight);
const isOpen   = expanded===date;
const accentCol= isDuty ? meta.color : (hasOv ? C.amber : C.green);

```
    return(
      <div key={date} style={{marginBottom:6,borderRadius:14,overflow:"hidden",
        border:`1px solid ${accentCol}35`,background:C.card}}>

        {/* Tappable header row — date on top, sectors below */}
        <button onClick={()=>setExpanded(isOpen?null:date)}
          style={{width:"100%",background:"none",border:"none",cursor:"pointer",
            padding:isDuty?"8px 14px":"10px 14px 10px",textAlign:"left",display:"block"}}>

          {isDuty ? (
            /* ── Duty day: single line, all inline ── */
            <div style={{display:"flex",alignItems:"center",gap:8,width:"100%"}}>
              <div style={{paddingRight:"28%",flexShrink:0}}>
              <span style={{
                fontFamily:"'Horizon','Arial Narrow',sans-serif",
                fontWeight:800,fontSize:"8px",textTransform:"uppercase",
                letterSpacing:"1px",flexShrink:0,display:"inline-block",
                transform:"scaleX(1.55)",transformOrigin:"left center",
                background:"linear-gradient(90deg,#FF6B35,#FFB347)",
              ...
              {/* Duty badge */}
              <div style={{background:`${meta.color}18`,border:`1px solid ${meta.color}35`,
                borderRadius:6,padding:"2px 8px",flexShrink:0,
                display:"flex",alignItems:"center",gap:4}}>
                <span style={{fontSize:10}}>{meta.icon}</span>
                <span style={{fontSize:10,fontWeight:800,color:meta.color}}>{entry.code}</span>
              </div>
              {/* Label — truncates, never overflows */}
              <span style={{fontSize:9,color:C.muted,flex:1,minWidth:0,
                overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                {meta.label}
              </span>
            </div>
          ) : (
            /* ── Flight day: date row + chips row ── */
            <div style={{width:"100%"}}>
              {/* Date row */}
              <div style={{marginBottom:7,display:"flex",
                alignItems:"center",justifyContent:"space-between"}}>
                <div style={{paddingRight:"30%",flexShrink:0}}>
                <span style={{
                  fontFamily:"'Horizon','Arial Narrow',sans-serif",
                  fontWeight:800,fontSize:"8px",textTransform:"uppercase",
                ...
                <div style={{display:"flex",alignItems:"center",gap:5}}>
                  {totalH>0&&(
                    <span style={{fontSize:9,color:C.muted,fontWeight:600}}>{totalH.toFixed(1)}h</span>
                  )}
                  {hasOv&&<span style={{fontSize:9}}>🌙</span>}
                  <span style={{fontSize:12,color:C.muted,
                    transform:isOpen?"rotate(90deg)":"none",
                    transition:"transform .2s",display:"inline-block",lineHeight:1}}>›</span>
                </div>
              </div>
              {/* Sector chips */}
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                {flights.map((f,fi)=>(
                  <div key={f.id||fi} style={{
                    background:f.overnight?C.amber+"18":C.green+"18",
                    border:`1px solid ${f.overnight?C.amber+"45":C.green+"45"}`,
                    borderRadius:8,padding:"4px 10px",
                    display:"flex",alignItems:"center",gap:5}}>
                    <span style={{fontSize:11,fontWeight:800,
                      color:f.overnight?C.amber:C.green}}>{f.flightNo}</span>
                    <span style={{fontSize:10,color:C.soft,fontWeight:500}}>{f.from}→{f.to}</span>
                    <span style={{fontSize:9,color:C.muted}}>{f.dep}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        {isOpen&&!isDuty&&(
          <div style={{borderTop:`1px solid ${C.border}30`,padding:"10px 14px 12px"}}>
            {flights.map((f,fi)=>(
              <button key={f.id||fi} onClick={()=>onSelect(f)}
                style={{width:"100%",background:f.overnight?C.amber+"10":C.green+"10",
                  border:`1px solid ${f.overnight?C.amber+"40":C.green+"40"}`,
                  borderRadius:11,padding:"11px 14px",marginBottom:fi<flights.length-1?8:0,
                  cursor:"pointer",textAlign:"left",display:"block"}}>
                <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:5}}>
                  <span style={{fontSize:13,fontWeight:900,
                    color:f.overnight?C.amber:C.green}}>{f.flightNo}</span>
                  <span style={{fontSize:12,color:C.soft}}>{f.from} → {f.to}</span>
                  {f.overnight&&<span style={{fontSize:10,color:C.amber}}>🌙+1</span>}
                ...
                <div style={{display:"flex",gap:16}}>
                  <div>
                    <div style={{fontSize:8,color:C.muted,letterSpacing:1,marginBottom:1}}>DEP</div>
                    <div style={{fontSize:12,fontWeight:700}}>{f.dep||"–"}</div>
                  </div>
                  <div>
                    <div style={{fontSize:8,color:C.muted,letterSpacing:1,marginBottom:1}}>ARR</div>
                    <div style={{fontSize:12,fontWeight:700}}>{f.arr||"–"}</div>
                  </div>
                  {f.dutyHours>0&&(
                    <div>
                      <div style={{fontSize:8,color:C.muted,letterSpacing:1,marginBottom:1}}>BLOCK</div>
                      <div style={{fontSize:12,fontWeight:700}}>{f.dutyHours.toFixed(1)}h</div>
                    </div>
                  )}
                  <div style={{marginLeft:"auto",textAlign:"right"}}>
                    <div style={{fontSize:8,color:C.muted,letterSpacing:1,marginBottom:1}}>SIGN OFF</div>
                    <div style={{fontSize:12,fontWeight:700,color:f.overnight?C.amber:C.text}}>
                      {f.signOff||f.arr||"–"}
                    </div>
                  </div>
                </div>
        ...
        {isOpen&&isDuty&&(
          <div style={{borderTop:`1px solid ${C.border}30`,
            padding:"10px 14px 12px",
            display:"flex",gap:12}}>
            {entry.signOn&&(
              <div>
                <div style={{fontSize:8,color:C.muted,letterSpacing:1,marginBottom:2}}>SIGN ON</div>
                <div style={{fontSize:13,fontWeight:700}}>{entry.signOn}</div>
              </div>
            )}
            {entry.signOff&&(
              <div>
                <div style={{fontSize:8,color:C.muted,letterSpacing:1,marginBottom:2}}>SIGN OFF</div>
                <div style={{fontSize:13,fontWeight:700}}>{entry.signOff}</div>
              </div>
            )}
            {entry.paidHours>0&&(
              <div>
                <div style={{fontSize:8,color:C.muted,letterSpacing:1,marginBottom:2}}>PAID</div>
                <div style={{fontSize:13,fontWeight:700,color:meta.color}}>{entry.paidHours}h</div>
              </div>
            )}
          </div>
        )}
...
const [rView,setRView]=useState(“grid”); // “list” | “grid”
...
<div style={{display:“flex”,gap:8,alignItems:“center”}}>
{/* View toggle */}
<div style={{display:“flex”,background:C.card,borderRadius:8,padding:3,
border:`1px solid ${C.border}`}}>
{[{k:“list”,ic:“☰”},{k:“grid”,ic:“⊞”}].map(({k,ic})=>(
<button key={k} onClick={()=>setRView(k)}
style={{width:30,height:26,borderRadius:6,border:“none”,
background:rView===k?C.card2:“transparent”,
color:rView===k?C.text:C.muted,fontSize:14,cursor:“pointer”,
transition:“all .15s”}}>{ic}</button>
))}
</div>
...
```
  {Object.keys(allDays).length===0?(
    <div style={{textAlign:"center",padding:"60px 24px"}}>
      ...
      <div style={{fontSize:13,color:C.soft,marginBottom:24,lineHeight:1.6}}>
        Import your CWP roster PDF to get started.
      </div>
    ...
    /* ── GRID VIEW ── */
    <GridView allDays={allDays} onSelect={onSelect}/>
  )}
</div>
```
...
//  IMPORT FLOW
// ═══════════════════════════════════════════════════════════════════
// ── CrewMate Server integration ─────────────────────────────────────────
const SERVER_URL = “https://crewmate-server-production.up.railway.app”;

async function cwpLogin(username, password) {
...
async function cwpFetchRoster(token) {
const res = await fetch(SERVER_URL + “/roster”, {
headers: { “Authorization”: “Bearer “ + token },
});
const data = await res.json();
if (!res.ok) throw new Error(data.error || “Could not fetch roster”);
return data.roster;
}

function parseServerRoster(serverRoster) {
const MS=[””,“Jan”,“Feb”,“Mar”,“Apr”,“May”,“Jun”,“Jul”,“Aug”,“Sep”,“Oct”,“Nov”,“Dec”];
const DM={MON:“Mon”,TUE:“Tue”,WED:“Wed”,THU:“Thu”,FRI:“Fri”,SAT:“Sat”,SUN:“Sun”};
const duties = serverRoster.duties || [];
const flights = []; const agenda = {};
let rgdCount=0, phoCount=0;
const month = serverRoster.month || 4;
const year  = serverRoster.year  || 2026;

duties.forEach(function(d) {
const dayM = d.date && d.date.match(/^(\d{2})(MON|TUE|WED|THU|FRI|SAT|SUN)$/);
if (!dayM) return;
const day = parseInt(dayM[1]);
const dow = dayM[2];
const dateStr = DM[dow] + “ “ + day + “ “ + MS[month];
agenda[dateStr] = agenda[dateStr] || [];
if (d.type === “flight”) {
flights.push({
date:dateStr, flightNo:d.flightNo,
from:d.from||“AKL”, to:d.to||”???”,
dep:d.signOn||d.atd||””, arr:d.ata||””,
signOff:d.signOff||””, aircraft:“A320”,
dutyHours:0, role:“CM”, overnight:false,
});
agenda[dateStr].push({type:“flight”,flightNo:d.flightNo,
from:d.from||“AKL”,to:d.to||”???”,
dep:d.signOn||””,arr:d.ata||””,
color:C.green,overnight:false});
} else {
if (d.code===“RGD”) rgdCount++;
if (d.code===“PHO”) phoCount++;
const meta = DUTY_META[d.code]||{color:C.muted,paidHours:0};
agenda[dateStr].push({type:“duty”,code:d.code,
color:meta.color,paidHours:meta.paidHours||0,
signOn:d.signOn,signOff:d.signOff});
}
});

const blhParts = (serverRoster.blh||“0:00”).split(”:”);
const flightBLH = parseInt(blhParts[0]||0)+parseInt(blhParts[1]||0)/60;
const groundPaidHours = (rgdCount+phoCount)*7.5;
return {
flights,agenda,
crewName:serverRoster.crewName||””,
empId:serverRoster.empId||””,
crewRole:“CM”,
totalBLH:flightBLH+groundPaidHours,
flightBLH,rgdCount,phoCount,groundPaidHours,
rosterMonth:month,rosterYear:year,
};
}

function CWPSyncModal({onClose, onImport}) {
const [step,     setStep]    = useState(“login”);
const [username, setUsername]= useState(function(){
try{return localStorage.getItem(“cwp_user”)||””;}catch(e){return “”;}
});
...
setErr(“Enter your CWP username and password.”); return;
}
setStep(“syncing”); setErr(””);
try {
setStatus(“Connecting to CWP…”);
const token = await cwpLogin(username.trim(), password);
try{localStorage.setItem(“cwp_user”, username.trim());}catch(e){}
setStatus(“Fetching your roster…”);
const roster = await cwpFetchRoster(token);
setStatus(“Parsing…”);
const parsed = parseServerRoster(roster);
if (!parsed.flights.length && !Object.keys(parsed.agenda).length) {
throw new Error(“Roster fetched but no duties found. CWP may have changed its HTML.”);
}
setStep(“done”);
setTimeout(function(){ onImport(parsed); onClose(); }, 1200);
} catch(e) {
setErr(e.message||String(e)); setStep(“error”);
}
};

return (
<div style={{position:“fixed”,inset:0,background:”#000000F0”,
backdropFilter:“blur(20px)”,zIndex:200,
display:“flex”,flexDirection:“column”,justifyContent:“flex-end”}}>
<div style={{background:C.card,borderRadius:“24px 24px 0 0”,padding:“24px 24px 52px”}}>
...
{step===“login”?“Sync from CWP”:step===“syncing”?“Syncing…”:
step===“done”?“Roster Synced ✅”:“Sync Failed”}
</div>
</div>
<button onClick={onClose} style={{background:C.card2,border:“1px solid “+C.border,
borderRadius:12,color:”#FF3B30”,fontSize:18,width:40,height:40,cursor:“pointer”,
...
```
    {step==="login"&&(
      <div>
        <div style={{background:C.amber+"12",border:"1px solid "+C.amber+"30",
          borderRadius:12,padding:"10px 14px",marginBottom:18,fontSize:11,
          color:C.soft,lineHeight:1.7}}>
          🔒 Your password is used once to log in and then discarded. It is never stored.
        </div>
        <div style={{marginBottom:12}}>
          <div style={{fontSize:10,color:C.muted,letterSpacing:1.5,marginBottom:6}}>CWP USERNAME</div>
          <input value={username} onChange={function(e){setUsername(e.target.value);}}
            placeholder="e.g. 674953"
            style={{width:"100%",background:C.card2,
              border:"2px solid "+(username?C.blue:C.border),
              borderRadius:12,padding:"13px 16px",color:C.text,fontSize:16,
              outline:"none",boxSizing:"border-box"}}/>
        </div>
        <div style={{marginBottom:18}}>
          <div style={{fontSize:10,color:C.muted,letterSpacing:1.5,marginBottom:6}}>CWP PASSWORD</div>
          <input type="password" value={password}
            onChange={function(e){setPassword(e.target.value);}}
            onKeyDown={function(e){if(e.key==="Enter")doSync();}}
            placeholder="Your CWP password"
            style={{width:"100%",background:C.card2,
              border:"2px solid "+(password?C.accent:C.border),
              borderRadius:12,padding:"13px 16px",color:C.text,fontSize:16,
              outline:"none",boxSizing:"border-box"}}/>
        </div>
        {err&&<div style={{color:"#FF3B30",fontSize:12,marginBottom:12}}>{err}</div>}
        <button onClick={doSync}
          style={{width:"100%",background:C.accent,border:"none",borderRadius:14,
            padding:"15px 0",color:"#fff",fontSize:15,fontWeight:700,cursor:"pointer"}}>
          Sync Roster Now →
        </button>
      </div>
    )}
    ...
    {step==="syncing"&&(
      <div style={{textAlign:"center",padding:"24px 0"}}>
        ...
        <div style={{fontSize:14,color:C.soft}}>{status}</div>
      </div>
    )}
    ...
    {step==="done"&&(
      <div style={{textAlign:"center",padding:"24px 0"}}>
        ...
        <div style={{fontSize:16,fontWeight:700}}>Roster synced!</div>
        <div style={{fontSize:12,color:C.muted,marginTop:6}}>Closing...</div>
      </div>
    )}

    {step==="error"&&(
      <div>
        <div style={{background:"#FF3B3015",border:"1px solid #FF3B3040",
          borderRadius:12,padding:16,marginBottom:16}}>
          <div style={{fontSize:13,fontWeight:700,color:"#FF3B30",marginBottom:6}}>Sync failed</div>
          <div style={{fontSize:12,color:C.soft,lineHeight:1.6}}>{err}</div>
        </div>
        <button onClick={function(){setStep("login");setPassword("");}}
          style={{width:"100%",background:C.card2,border:"1px solid "+C.border,
            borderRadius:14,padding:"13px 0",color:C.text,
...
const [err,     setErr]   =useState(””);
const [status,  setStatus]=useState(””);
const [showCWP, setShowCWP]=useState(false);
...
action:{label:“Open CWP →”,fn:()=>window.open(“https://roc.jetstar.com/CWP_WA/”,”_blank”)}},
{n:“2”,icon:“📥”,col:C.green, title:“Save PDF with Details”},
{n:“3”,icon:“⬆️”,col:C.accent,title:“Upload Here”,
...
{step===“guide”&&(
<div>
{showCWP&&<CWPSyncModal onClose={()=>setShowCWP(false)}
onImport={function(parsed){onImport(parsed);onClose();}}/>}
<div style={{background:“linear-gradient(135deg,”+C.accent+“18,”+C.blue+“12)”,
border:“1px solid “+C.accent+“35”,borderRadius:16,padding:16,marginBottom:16}}>
<div style={{fontSize:13,fontWeight:700,marginBottom:6}}>⚡ One-Tap Sync</div>
<div style={{fontSize:11,color:C.soft,lineHeight:1.6,marginBottom:12}}>
Log in with your CWP credentials — your roster imports automatically.
Password never stored.
</div>
<button onClick={function(){setShowCWP(true);}}
...
<div style={{fontSize:10,color:C.muted,whiteSpace:“nowrap”}}>OR UPLOAD PDF MANUALLY</div>
<div style={{flex:1,height:1,background:C.border}}/>
</div>
{STEPS.map(gs=>(
<div key={gs.n} style={{background:C.card,border:`1px solid ${C.border}`,
borderRadius:12,padding:“10px 10px 10px 11px”,marginBottom:8,
position:“relative”,overflow:“hidden”}}>
<div style={{position:“absolute”,left:0,top:0,bottom:0,width:3,background:gs.col}}/>
<div style={{display:“flex”,alignItems:“center”,gap:10}}>
<div style={{width:30,height:30,borderRadius:8,background:`${gs.col}20`,
border:`1px solid ${gs.col}40`,display:“flex”,alignItems:“center”,
justifyContent:“center”,fontSize:14,flexShrink:0}}>{gs.icon}</div>
<div style={{flex:1}}>
<div style={{fontSize:13,fontWeight:700}}>{gs.title}</div>
</div>
{gs.action&&<button onClick={gs.action.fn} style={{
background:`${gs.col}20`,border:`1px solid ${gs.col}50`,borderRadius:9,
padding:“6px 12px”,color:gs.col,fontSize:11,fontWeight:700,
...
const [cycle,     setCycle]   =useState(“paycycle”); // “paycycle” | “monthly”
...
const rMonth=crew.rosterMonth||4;
const rYear=crew.rosterYear||2026;
const safeRoster=Array.isArray(roster)?roster:[];
const safeAgenda=(agenda&&typeof agenda===“object”)?agenda:{};

const cycleStart=new Date(rYear,rMonth-2,21);
const cycleEnd=new Date(rYear,rMonth-1,20);
const monthStart=new Date(rYear,rMonth-1,1);
const monthEnd=new Date(rYear,rMonth-1,31);

const fmtD=d=>d.getDate()+” “+MS2[d.getMonth()];
const cycleLabel=fmtD(cycleStart)+” – “+fmtD(cycleEnd);
const monthLabel=MS2[rMonth-1]+” “+rYear;

const inCycle=function(dateStr){
if(!dateStr||typeof dateStr!==“string”)return true;
const p=dateStr.split(” “);
if(p.length<3)return true;
const mon=MN[p[2]];
if(!mon)return true;
const d=new Date(rYear,mon-1,Number(p[1])||1);
if(cycle===“monthly”)return d>=monthStart&&d<=monthEnd;
return d>=cycleStart&&d<=cycleEnd;
};

const cycleRoster=safeRoster.filter(function(f){return inCycle(f.date);});
const cycleFlightBLH=cycleRoster.reduce(function(s,f){return s+(f.dutyHours||0);},0);

let cycleRGD=0,cyclePHO=0,cycleAVL=0;
Object.entries(safeAgenda).forEach(function(entry){
const dateStr=entry[0];
const items=entry[1];
if(!inCycle(dateStr))return;
if(!Array.isArray(items))return;
items.forEach(function(i){
if(i.type!==“duty”)return;
if(i.code===“RGD”)cycleRGD++;
if(i.code===“PHO”)cyclePHO++;
if(i.code===“AVL”)cycleAVL++;
});
});

const cycleGroundH=(cycleRGD+cyclePHO)*7.5;
const cycleTotalH=cycleFlightBLH+cycleGroundH;
const targetPay=pay.targetPay||5000;
const targetHours=pay.targetHours||80;
const avlEarned=cycleAVL*(pay.sbRate||0)*7.5;
const earned=cycleTotalH*(pay.rate||0)+avlEarned;
const pLeft=Math.max(0,targetPay-earned);
const hLeft=Math.max(0,targetHours-cycleTotalH);
const pct=Math.min(targetPay>0?(earned/targetPay)*100:0,100);
...
```
  {view==="pay"&&(
    <div>
      {/* Cycle toggle */}
      <div style={{display:"flex",background:C.card,borderRadius:12,
        padding:4,marginBottom:14,border:`1px solid ${C.border}`}}>
        {[{k:"paycycle",l:"PAY CYCLE"},{k:"monthly",l:"MONTHLY"}].map(({k,l})=>(
          <button key={k} onClick={()=>setCycle(k)} style={{flex:1,padding:"8px 0",
            borderRadius:9,border:"none",
            background:cycle===k?C.accent:"transparent",
            color:cycle===k?"#fff":C.muted,
      ...
      <div style={{textAlign:"center",marginBottom:16}}>
        <div style={{fontSize:11,color:C.muted,letterSpacing:1}}>
          {cycle==="paycycle"?"PAY CYCLE":"CALENDAR MONTH"}
        </div>
        <div style={{fontSize:13,fontWeight:700,color:C.soft,marginTop:2}}>
          {cycle==="paycycle"?cycleLabel:monthLabel}
        </div>
      </div>
      ...
      <div style={{background:C.card,borderRadius:16,padding:"14px 18px",
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
          {[
            {l:"FLIGHT HRS", v:`${cycleFlightBLH.toFixed(1)}h`, c:C.green},
            {l:"GROUND PAID",v:`${cycleGroundH.toFixed(1)}h`,   c:C.blue},
            {l:"TOTAL PAID", v:`${cycleTotalH.toFixed(1)}h`,    c:C.accent},
            {l:"TOTAL EARN", v:`$${earned.toFixed(0)}`,          c:C.amber},
          ].map(({l,v,c})=>(
      ...
      <div style={{background:C.card,borderRadius:22,padding:24,border:`1px solid ${C.border}`,
        ...
        <div style={{display:"flex",gap:16,width:"100%",marginTop:16}}>
          {[{l:"PAID HRS",v:`${cycleTotalH.toFixed(1)}h`,c:C.accent},
          ...
          <div style={{background:`${C.green}10`,border:`1px solid ${C.green}30`,
            ...
            <div style={{fontSize:14,fontWeight:700,color:C.green}}>
              ...
              <span style={{fontSize:10,color:C.soft,fontWeight:400}}>
                {" "}· {cycleAVL} day{cycleAVL>1?"s":""} × 7.5h × ${(pay.sbRate||0).toFixed(2)}/hr
              </span>
            </div>
          </div>
        )}
