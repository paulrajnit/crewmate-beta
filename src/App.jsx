...
// paidHours: set for codes that generate pay (RGD=7.5h, PHO=7.5h per Jetstar NZ agreement)
...
function parseCWPRoster(rawText){
  ...
  const nameM=rawText.match(/([A-Z]{2,}\s+[A-Z]{2,})\s+(\d{5,7})\s+E-/);
  const crewName=nameM?nameM[1].split(" ").map(w=>w[0]+w.slice(1).toLowerCase()).join(" "):"";
  const empId=nameM?nameM[2]:"";
  const roleM=rawText.match(/[A-Z]{3}-([A-Z]{2,4})\s+\d{2}[A-Z]{3}/);
  ...
  const NOT_IATA=new Set(["JQ","NEO","BLH","RGD","PHO","OFF","STR","ATA","ATD","REQ","RQD","PL",
    ...
    "STB","MAT","TVL","TCH","HTC","PAS","HLV","320","321","787","CM","CM2","DD","CI","CO","AC"]);
  ...
  const DATE_RE=/^(\d{2})(MON|TUE|WED|THU|FRI|SAT|SUN)(.*)/;
  ...
  const getIatas=txt=>{const r=[];const re=/\b([A-Z]{3})\b/g;let m;
    while((m=re.exec(txt)))if(!NOT_IATA.has(m[1]))r.push(m[1]);return r;};
  const getTimes=txt=>{const r=[];const re=/\b(\d{2}:\d{2})\b/g;let m;
    while((m=re.exec(txt)))if(m[1]!=="00:00"&&m[1]!=="24:00")r.push(m[1]);return r;};
  const lines=rawText.split("\n").map(l=>l.trim()).filter(Boolean);
  // Build day groups
  const dayGroups=[];let cur=null;
  for(const line of lines){
    ...
    if(dm){
      if(cur)dayGroups.push(cur);
      cur={dateStr:`${DL[dm[2]]} ${parseInt(dm[1])} ${MS[rosterMonth]}`,lines:[],dayType:null};
      if(dm[3].trim())cur.lines.push(dm[3].trim());
  ...
  for(const dg of dayGroups){
    for(const l of dg.lines){
      if(FLT_RE.test(l)&&!l.startsWith("PCK")&&!l.startsWith("HTL")){
        const fm=FLT_RE.exec(l);
        const rest=l.slice(fm.index+fm[0].length);
        const times=getTimes(rest);const iatas=getIatas(rest);
        if(times.length>0&&!(iatas.length===0&&times.length===1)){dg.dayType="FLIGHT";break;}
      }
      if(!dg.dayType)for(const c of DUTY_SET)
        if(new RegExp(`\\b${c}\\b`).test(l)&&!FLT_RE.test(l)){dg.dayType=c;break;}
    }
  }
  let rgdCount=0,phoCount=0;
  const flightsByDay=[],carryForward=[];
  for(let di=0;di<dayGroups.length;di++){
    const dg=dayGroups[di];
    if(DUTY_SET.has(dg.dayType||"")){
      if(dg.dayType==="RGD")rgdCount++;
      if(dg.dayType==="PHO")phoCount++;
      for(const l of dg.lines){
        if(!FLT_RE.test(l)||l.startsWith("PCK")||l.startsWith("HTL"))continue;
        const fm=FLT_RE.exec(l);const fno=fm[1];
        const rest=l.slice(fm.index+fm[0].length);
        const iatas=getIatas(rest);const times=getTimes(rest);
        if(times.length===0||(iatas.length===0&&times.length===1))continue;
        carryForward.push({no:fno,from:iatas[0]||null,to:null,dep:times[0],arr:null,overnight:true});
      }
      continue;
    }
    if(dg.dayType!=="FLIGHT")continue;
    const dayFlights={};let layover=false;
    for(const cf of carryForward.splice(0))if(!dayFlights[cf.no])dayFlights[cf.no]={...cf};
    for(const l of dg.lines){
      if(!l)continue;
      const first=l.split(" ")[0];
      if(first==="HTL"){layover=true;continue;}
      if(first==="PCK")continue;
      if(!FLT_RE.test(l))continue;
      const fm=FLT_RE.exec(l);const fno=fm[1];
      const rest=l.slice(fm.index+fm[0].length);
      const iatas=getIatas(rest);const rawTimes=getTimes(rest);
      if(iatas.length===0&&rawTimes.length===1)continue;
      if(!dayFlights[fno]){
        const dep=rawTimes[0]||null;
        const times=[...rawTimes];
        while(times.length>1&&isBLH(times[times.length-1],dep))times.pop();
        const frm=iatas[0]||null;const to=iatas.length>1?iatas[1]:null;
        const arr=times.length>=2?times[1]:null;
        dayFlights[fno]={no:fno,from:frm,to,dep,arr:arr||null,overnight:!arr};
      }else{
        const ex=dayFlights[fno];const dep=ex.dep;
        if(iatas.length>0&&!ex.to)ex.to=iatas[0];
        const times=[...rawTimes];
        while(times.length>0&&isBLH(times[times.length-1],dep))times.pop();
        if(times.length>0&&!ex.arr){ex.arr=times[0];ex.overnight=false;}
      }
    }
    const list=Object.values(dayFlights).filter(f=>f.dep);
    if(list.length)flightsByDay.push({dateStr:dg.dateStr,layover,flights:list});
  }
  // Link overnights: find continuation on next day (dep < 08:00 = early arrival)
  for(let i=0;i<flightsByDay.length-1;i++){
    const today=flightsByDay[i],tomorrow=flightsByDay[i+1];
    for(const f of today.flights){
      if(!f.overnight||f.arr)continue;
      const tf=tomorrow.flights.find(x=>x.no===f.no&&x.dep&&t2m(x.dep)<8*60);
      if(tf){
        f.arr=tf.arr||tf.dep;
        if(!f.to&&tf.from)f.to=tf.from;
        tf._cont=true;
      }
    }
    tomorrow.flights=tomorrow.flights.filter(x=>!x._cont);
  }
  const flights=[],agenda={};
  for(const day of flightsByDay){
    agenda[day.dateStr]=agenda[day.dateStr]||[];
    const signOn=day.flights[0]?.dep||null;
    const signOff=day.flights[day.flights.length-1]?.arr||null;
    for(const f of day.flights){
      if(!f.dep)continue;
      const dm=t2m(f.arr)-t2m(f.dep);const dh=f.arr&&dm>0?+(dm/60).toFixed(2):0;
      ...
      agenda[day.dateStr].push({type:"flight",flightNo:f.no,from:f.from||"AKL",
        to:f.to||"???",dep:f.dep,arr:f.arr||"",signOff:signOff||"",signOn:signOn||"",
        color:(f.overnight||false)?C.amber:C.green,overnight:f.overnight||false});
    }
  }
  let rgdIdx=0;
  for(const dg of dayGroups){
    if(!DUTY_SET.has(dg.dayType||""))continue;
    agenda[dg.dateStr]=agenda[dg.dateStr]||[];
    let sOn=null,sOff=null;
    for(const l of dg.lines){
      const times=getTimes(l);
      if(times.length>=2&&!sOn){sOn=times[0];sOff=times[times.length-1];}
    }
    const meta=DUTY_META[dg.dayType]||{};
    agenda[dg.dateStr].push({type:"duty",code:dg.dayType,
      color:meta.color||C.muted,paidHours:meta.paidHours||0,signOn:sOn,signOff:sOff});
  }
  ...
  return{flights,agenda,crewName,empId,crewRole,totalBLH,flightBLH,
    rgdCount,phoCount,groundPaidHours,rosterMonth,rosterYear};
}
...
const AIRCRAFT_PROFILE={
  ...
  operator:"Jetstar Airways", modeS:"7C6B0B",
...
async function fetchFlightData(flightNo){
  ...
  const d=DEMO_FLIGHTS[flightNo];
  return d?{...d,fetchedAt:new Date().toLocaleTimeString("en-NZ",{hour:"2-digit",minute:"2-digit"})}:null;
}
...
const DEFAULT_PAY  ={rate:68.50,targetHours:80,targetPay:5000};
const DEFAULT_BILLS=[
...
function OnboardingScreen({onDone}){
  ...
  const [step, setStep]=useState(0); // 0=welcome, 1=name, 2=done
  ...
  const submit=()=>{
    ...
    try{localStorage.setItem("crewmate_nick",name);}catch(e){}
    onDone(name);
  };
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",maxWidth:430,margin:"0 auto",
      ...
      {/* Ambient glows */}
      <div style={{position:"fixed",top:-100,right:-80,width:300,height:300,
        background:`radial-gradient(circle,${C.accent}20,transparent 70%)`,pointerEvents:"none"}}/>
      <div style={{position:"fixed",bottom:60,left:-80,width:260,height:260,
        background:`radial-gradient(circle,${C.blue}15,transparent 70%)`,pointerEvents:"none"}}/>

      {step===0&&(
        <div style={{animation:"fadein .5s ease",position:"relative"}}>
          ...
          <div style={{fontSize:12,letterSpacing:3,color:C.accent,fontWeight:700,marginBottom:12}}>
            CREWMATE
          </div>
          ...
          <button onClick={()=>setStep(1)} style={{
            background:C.accent,border:"none",borderRadius:16,
            padding:"16px 0",width:"100%",color:"#fff",
          ...
          <div style={{marginTop:16,fontSize:11,color:C.muted,textAlign:"center"}}>
            For Jetstar Cabin Crew
          </div>
        </div>
      )}

      {step===1&&(
        <div style={{animation:"fadein .4s ease",position:"relative"}}>
          <style>{`@keyframes fadein{from{opacity:0;transform:translateY(20px)}to{opacity:1;transform:none}}`}</style>
          ...
          <input
            ...
            onKeyDown={e=>e.key==="Enter"&&nick.trim()&&submit()}
...
function SettingsSheet({crew, nick, onClose, onSaveNick, onSaveRole}){
      ...
      <div style={{background:C.card,borderRadius:"24px 24px 0 0",
        ...
        <div style={{padding:"8px 24px 16px",borderBottom:`1px solid ${C.border}`,
          ...
          <button onClick={onClose} style={{background:C.card2,border:`1px solid ${C.border}`,
            borderRadius:12,color:C.soft,fontSize:18,width:40,height:40,cursor:"pointer",
          ...
          {page==="menu"&&(
            <div>
              ...
              <div style={{marginTop:10,padding:"12px 0",textAlign:"center"}}>
                <div style={{fontSize:10,color:C.muted}}>CrewMate · Jetstar NZ Cabin Crew</div>
          ...
          {page==="profile"&&(
            <div>
              <div style={{fontSize:13,color:C.soft,marginBottom:20,lineHeight:1.6}}>
                Your designation is used throughout the app — crew manifests, pay calculations, and roster display.
              </div>
              <div style={{fontSize:10,color:C.muted,letterSpacing:2,marginBottom:12}}>SELECT DESIGNATION</div>
              {DESIGNATIONS.map(d=>{
                ...
                return(
                  <button key={d.code} onClick={()=>setRoleEdit(d.code)}
                    ...
                    <div style={{width:40,height:40,borderRadius:10,
                      ...
                      display:"flex",alignItems:"center",justifyContent:"center",
                      fontSize:15,fontWeight:900,color:dc,flexShrink:0}}>{d.code}</div>
                    <div style={{flex:1}}>
                      <div style={{fontSize:14,fontWeight:700,color:active?dc:C.text}}>{d.label}</div>
                      <div style={{fontSize:11,color:C.soft,marginTop:2}}>{d.desc}</div>
                    </div>
                    {active&&<div style={{fontSize:16,color:dc}}>✓</div>}
                  </button>
                );
              })}
              <button onClick={()=>onSaveRole(roleEdit)}
                style={{width:"100%",background:roleColor,border:"none",
                  borderRadius:14,padding:"15px 0",marginTop:6,
          ...
          {page==="siri"&&(
            <div>
              ...
              {[
                ...
                {n:"2",icon:"🔗",title:"Create New Shortcut",body:'Tap + in Shortcuts app. Add action: "Open URL". Enter: https://roc.jetstar.com/CWP_WA/'},
            ...
            <div>
              ...
              <div style={{background:C.card2,border:`1px solid ${C.border}`,
                ...
                {[
                  ...
                  "CrewMate is an independent tool and is not affiliated with, endorsed by, or connected to Jetstar Airways or Qantas Group.",
                ].map((t,i)=>(
              ...
              <div style={{background:`${C.amber}10`,border:`1px solid ${C.amber}30`,
                ...
                <div style={{fontSize:11,color:C.soft,lineHeight:1.6}}>
                  ...
                  (roc.jetstar.com). Never share your roster PDF with
...
export default function CrewMate(){
  ...
  const [crew,     setCrew]   =useState({crewName:"Paul Rajnit",empId:"674953",
    totalBLH:DEMO_TOTAL_BLH,flightBLH:DEMO_FLIGHT_BLH,role:"CM",
    rgdCount:6,phoCount:2,groundPaidHours:60});
  ...
  const handleImport=({flights,agenda:ag,crewName,empId,crewRole,totalBLH,flightBLH,rgdCount,phoCount,groundPaidHours})=>{
    setRoster(flights.map((f,i)=>({...f,id:Date.now()+i,
      statusColor:f.overnight?C.amber:C.green,status:f.overnight?"Multi-day":"On Time"})));
    setAgenda(ag||{});
    if(crewName)setCrew({crewName,empId,totalBLH,flightBLH,role:crewRole||"CC",rgdCount:rgdCount||0,phoCount:phoCount||0,groundPaidHours:groundPaidHours||0});
    setModal(false);
    const ts = new Date().toLocaleString("en-NZ",{day:"numeric",month:"short",hour:"2-digit",minute:"2-digit"});
    setLastSync(ts);
    try{localStorage.setItem("crewmate_last_sync",ts);}catch(e){}
  };
  ...
  if(!nick) return <OnboardingScreen onDone={name=>{setNick(name);}}/>;
  ...
  const getManifest=(flightNo,from,to,dateStr)=>getCrewForFlight(flightNo,from,to,crew.role,crew.crewName,crew.empId,dateStr);
  ...
  return(
    <div style={{background:C.bg,minHeight:"100vh",maxWidth:430,margin:"0 auto",
      ...
      {settings&&<SettingsSheet crew={crew} nick={nick}
      ...
      <div style={{padding:"52px 24px 0",position:"relative",zIndex:1}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          ...
          <div style={{textAlign:"right"}}>
            ...
            {lastSync&&<div style={{fontSize:9,color:C.muted,marginTop:4,textAlign:"center"}}>
              Updated {lastSync}
            </div>}
          </div>
        </div>
      ...
      <div style={{display:"flex",margin:"18px 24px 0",background:C.card,borderRadius:14,
        ...
        {TABS.map((t,i)=>(
          <button key={t} onClick={()=>setTab(i)} style={{flex:1,padding:"10px 0",borderRadius:10,
            border:"none",background:tab===i?C.accent:"transparent",
            color:tab===i?"#fff":C.muted,fontSize:10,fontWeight:700,
            letterSpacing:2,cursor:"pointer",transition:"all .2s"}}>{t}</button>
        ))}
      </div>

      <div ref={swipeRef} style={{padding:"18px 24px 100px",position:"relative",zIndex:1}}>
        {tab===0&&(sel
        ...
        {tab===1&&<TodayTab
          ...
          crewInfo={crew}
          onCrewTap={(fno,fr,to,dt)=>setCrewSheet(getManifest(fno,fr,to,dt))}/>}
...
function TodayTab({todayItems,tomorrowItems,crewInfo,onCrewTap}){
  ...
  return(
    <div>
      {regoSheet&&<RegoSheet profile={AIRCRAFT_PROFILE} onClose={()=>setRegoSheet(false)}/>}
      ...
      {todayItems.map((item,i)=>{
        ...
        return(
          <div key={i}>
            <FlightLiveCard
              item={item} live={live}
              expanded={expanded===item.flightNo}
              onToggle={()=>setExpanded(e=>e===item.flightNo?null:item.flightNo)}
              onRegoTap={()=>setRegoSheet(true)}
              onCrewTap={()=>onCrewTap(item.flightNo,item.from,item.to,item.date||TODAY)}
              loading={loading&&!live}/>
            {!isLast&&todayItems[i+1]?.type==="flight"&&(
              <div style={{background:C.card,border:`1px solid ${C.border}`,
                borderRadius:12,padding:"10px 16px",marginBottom:8,
                display:"flex",alignItems:"center",gap:10}}>
                <div style={{fontSize:14}}>🔄</div>
                <div style={{fontSize:11,color:C.soft}}>
                  Turnaround · {item.to} · Same aircraft · Crew brief
                </div>
              </div>
            )}
          </div>
      ...
      <div style={{marginTop:22}}>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12}}>
          <div style={{fontSize:10,fontWeight:700,letterSpacing:2,color:C.soft}}>TOMORROW</div>
          <div style={{fontSize:11,color:C.muted}}>{TOMORROW}</div>
        </div>
        {tomorrowItems.length===0?(
          <div style={{background:C.card,borderRadius:14,padding:"14px 18px",
            border:`1px solid ${C.border}`,textAlign:"center"}}>
            <div style={{fontSize:12,color:C.muted}}>Nothing scheduled</div>
          </div>
        ):tomorrowItems.map((item,i)=><DutyRow key={i} item={item}/>)}
      </div>
    </div>
  );
}
...
function TimeUntil({target}){
  ...
  useEffect(()=>{
    const calc=()=>{
      ...
      setMins(Math.round((then-now)/60000));
    };
    calc();const t=setInterval(calc,30000);return()=>clearInterval(t);
  },[target]);
  if(mins===null)return null;
  const hrs=Math.floor(mins/60),rem=mins%60;
  const col=mins<60?C.red:mins<180?C.amber:C.accent;
  return(
    <div style={{textAlign:"right"}}>
...
function FlightLiveCard({item,live,expanded,onToggle,onRegoTap,onCrewTap,loading}){
  const col=item.overnight?C.amber:C.green;
  ...
  return(
    <div style={{marginBottom:10}}>
      <div onClick={onToggle} style={{background:C.card,
        borderRadius:expanded?"16px 16px 0 0":"16px",
        padding:"16px 18px",border:`1px solid ${expanded?C.blue+"66":C.border}`,
        ...
        <div style={{marginLeft:10}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            ...
            <div style={{display:"flex",alignItems:"center",gap:7}}>
              <div style={{fontSize:11,fontWeight:700,color:statusCol}}>{status}</div>
              ...
              <div style={{fontSize:12,color:C.muted,transition:"transform .2s",
          ...
          {(live?.rego||item.rego)&&(
            <button onPointerDown={e=>e.stopPropagation()}
              onClick={e=>{e.stopPropagation();e.preventDefault();onRegoTap&&onRegoTap();}}
              ...
              <span style={{fontSize:12,fontWeight:700,color:"#FFFFFF",letterSpacing:.5}}>
                {live?.rego||item.rego}
              </span>
      ...
      {expanded&&(
        <div style={{background:C.card2,borderRadius:"0 0 16px 16px",
          border:`1px solid ${C.blue}44`,borderTop:"none",
          padding:"16px 18px",animation:"cwfade .2s ease"}}>
          {loading?<div style={{textAlign:"center",padding:"12px 0",color:C.muted,fontSize:13}}>Fetching live data…</div>:(
            <div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
                {[{l:"DEPT TERMINAL",v:live?.terminal_dep||"–"},{l:"DEPT GATE",v:live?.gate_dep||"–"},
                  {l:"ARR TERMINAL",v:live?.terminal_arr||"–"},{l:"ARR GATE",v:live?.gate_arr||"–"},
                  {l:"AIRCRAFT",v:live?.aircraft||"A320"},{l:"DISTANCE",v:live?.distance||"–"},
                  {l:"DURATION",v:live?.duration||"–"},
                  {l:"SIGN OFF",v:signOff?(signOff+(item.overnight?" +1":"")):"–"}].map(({l,v})=>(
                  <div key={l} style={{background:C.card,borderRadius:10,padding:12}}>
                    <div style={{fontSize:9,color:C.muted,letterSpacing:1.5,marginBottom:3}}>{l}</div>
                    <div style={{fontSize:13,fontWeight:700,color:C.text}}>{v}</div>
                  </div>
                ))}
              </div>
              <div style={{background:C.card,borderRadius:10,padding:12,display:"flex",gap:16}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:9,color:C.muted,letterSpacing:1.5,marginBottom:3}}>ON-TIME PERFORMANCE</div>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <div style={{fontSize:16,fontWeight:700,color:C.green}}>{live?.otpScore||"–"}/5</div>
                    <div style={{fontSize:11,color:C.soft}}>{live?.otpPct||""} on time</div>
                  </div>
                </div>
                <div style={{textAlign:"right"}}>
                  <div style={{fontSize:9,color:C.muted,letterSpacing:1.5,marginBottom:3}}>AVG DELAY</div>
                  <div style={{fontSize:14,fontWeight:700,color:C.amber}}>{live?.avgDelay||"–"}</div>
                </div>
              </div>
              <div style={{marginTop:8,fontSize:10,color:C.muted,textAlign:"center"}}>
                Auto-refreshes every 10 min · Tap ↻ to refresh now
              </div>
            </div>
          )}
        </div>
...
// ─── Aircraft rego sheet ──────────────────────────────────────────
function RegoSheet({profile,onClose}){
  return(
    <div style={{position:"fixed",inset:0,background:"#000000E0",
      ...
      <div style={{background:C.card,borderRadius:"24px 24px 0 0",padding:"0 0 40px",maxHeight:"88vh",overflowY:"auto"}}>
        ...
        <div style={{padding:"10px 24px 18px",borderBottom:`1px solid ${C.border}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              ...
              <div style={{fontSize:28,fontWeight:900,letterSpacing:-1,color:"#FFFFFF"}}>{profile.rego}</div>
              <div style={{fontSize:13,color:C.soft,marginTop:2}}>{profile.type} · {profile.operator}</div>
            </div>
            <button onClick={onClose} style={{background:C.card2,border:`1px solid ${C.border}`,
              borderRadius:12,color:C.soft,fontSize:18,width:40,height:40,cursor:"pointer",
              display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
        ...
        <div style={{padding:"20px 24px 0"}}>
          <div style={{fontSize:10,color:C.muted,letterSpacing:2,marginBottom:12}}>SPECS</div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
            {[{l:"TYPE",v:profile.type},{l:"MSN",v:profile.msn},{l:"BUILT",v:profile.built},
              {l:"AGE",v:profile.age},{l:"MODE-S",v:profile.modeS},{l:"ENGINES",v:profile.engines},
              {l:"MTOW",v:profile.mtow},{l:"MAX SPEED",v:profile.maxSpeed},
              {l:"RANGE",v:profile.range},{l:"SEATS",v:profile.seats}].map(({l,v})=>(
          ...
          <div style={{background:`${C.green}12`,border:`1px solid ${C.green}35`,borderRadius:14,padding:16}}>
            <div style={{fontSize:13,fontWeight:700,marginBottom:4,color:C.green}}>✅ Aircraft completed today's schedule</div>
            <div style={{fontSize:12,color:C.soft,lineHeight:1.6}}>
              All 5 prior sectors landed. VH-VFH is positioned in Newcastle (NTL) and scheduled to position to MEL before your JQ216 departure.
            </div>
          </div>
        </div>
      </div>
    ...
    <div>
      ...
      {Object.entries(allDays).map(([date,entry],gi)=>(
        <div key={date} style={{marginBottom:14}}>
          <div style={{fontSize:10,color:C.muted,letterSpacing:2,marginBottom:8}}>{date.toUpperCase()}</div>
          {entry.type==="duty"
...
function FlightDetail({flight:f,onBack,onCrewTap}){
  ...
  return(
    <div>
      <button onClick={onBack} style={{background:"none",border:"none",color:C.accent,
...
function ImportFlow({onClose,onImport}){
  ...
  const run=useCallback(async file=>{
    ...
    try{
      ...
      if(!parsed.flights.length)throw new Error("No flights found. Is this the Jetstar CWP PDF?");
  ...
  const STEPS=[
    {n:"1",icon:"🌐",col:C.blue,title:"Open Crew Web Portal",
     body:"Tap the button to open CWP in your browser. Log in with your Jetstar employee credentials. If the portal keeps refreshing, clear your browser cookies and try again.",
     action:{label:"Open CWP in Browser →",fn:()=>window.open("https://roc.jetstar.com/CWP_WA/","_blank")}},
          ...
          <div>
            ...
            <div onClick={()=>fileRef.current?.click()} style={{border:`2px dashed ${C.accent}55`,
              ...
              <div style={{fontSize:12,color:C.soft,lineHeight:1.6,marginBottom:18}}>
                Select the PDF from the Jetstar Crew Web Portal
              </div>
        ...
        {step==="confirm"&&result&&(
          <div>
            <div style={{background:`linear-gradient(135deg,${C.accent}22,${C.blue}18)`,
              ...
              <div style={{fontSize:12,color:C.soft}}>{result.empId} · Jetstar Airways</div>
              <div style={{marginTop:12,display:"flex",gap:16,flexWrap:"wrap"}}>
                ...
                <div>
                  <div style={{fontSize:9,color:C.muted,letterSpacing:1.5}}>GROUND PAY</div>
                  <div style={{fontSize:18,fontWeight:700,color:C.purple}}>{result.groundPaidHours?.toFixed(1)||"0"}h</div>
                </div>
                <div>
                  <div style={{fontSize:9,color:C.muted,letterSpacing:1.5}}>TOTAL PAID</div>
                  <div style={{fontSize:18,fontWeight:700,color:C.accent}}>{result.totalBLH?.toFixed(1)||"–"}h</div>
                </div>
              </div>
              {(result.rgdCount||result.phoCount)&&(
                <div style={{fontSize:11,color:C.muted,marginTop:6}}>
                  {result.rgdCount} RGD + {result.phoCount} PHO × 7.5h each
                </div>
              )}
            </div>
            ...
            <div style={{display:"flex",gap:10,marginTop:14}}>
              <button onClick={()=>setStep("upload")} style={{flex:1,background:C.card2,
                border:`1px solid ${C.border}`,borderRadius:14,padding:"14px 0",
                color:C.soft,fontSize:13,fontWeight:600,cursor:"pointer"}}>↩ Re-upload</button>
              <button onClick={()=>onImport(result)} style={{flex:2,background:C.accent,
                border:"none",borderRadius:14,padding:"14px 0",color:"#fff",
                fontSize:14,fontWeight:700,cursor:"pointer"}}>
                Import {result.flights.length} Flights →</button>
            </div>
          </div>
        )}
        {step==="error"&&(
...
function HoursTab({roster,totalDuty,crew,pay,setPay,bills,setBills,agenda}){
  ...
  const savePay=()=>{setPay(draft);setEditPay(false);};
        ...
        <div>
          <div style={{background:C.card,borderRadius:16,padding:"13px 18px",
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:11,color:C.soft}}>{crew.empId} · {crew.role} · JQ</div>
            </div>
          ...
          <div style={{background:C.card,borderRadius:16,padding:"14px 18px",
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>
              {[
                {l:"FLIGHT",v:`${(crew.flightBLH||0).toFixed(1)}h`,c:C.green},
                {l:"RGD/PHO",v:`${(crew.groundPaidHours||0).toFixed(1)}h`,c:C.blue,
                 sub:`${(crew.rgdCount||0)} RGD + ${(crew.phoCount||0)} PHO`},
          ...
          <div style={{background:C.card,borderRadius:18,padding:20,border:`1px solid ${C.border}`,marginBottom:12}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:12}}>
              {[{l:"PAY TARGET",v:`$${targetPay.toLocaleString()}`},
                ...
                {l:"HOURS TARGET",v:`${targetHours}h`}].map(({l,v})=>(
              ...
              <div key={bill.id} style={{background:C.card,borderRadius:14,padding:"13px 16px",marginBottom:8,border:`1px solid ${C.border}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                  ...
                  <button onClick={()=>deleteBill(bill.id)} style={{background:"none",border:`1px solid ${C.border}`,
                    borderRadius:8,padding:"4px 10px",color:C.muted,fontSize:12,cursor:"pointer"}}>✕</button>
                </div>
