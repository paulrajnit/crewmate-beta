...
function parseCWPRoster(rawText){
  ...
  for(let line of lines){
    ...
    if(!cur.flights[fno]){
      const dep=rawt[0]||null;
      const times=[...rawt];
      if(dep)while(times.length>1&&isBLH(times[times.length-1],dep))times.pop();
      const frm=iatas[0]||null;const to=iatas.length>1?iatas[1]:null;
      const arr=times.length>=2?times[1]:null;
      cur.flights[fno]={no:fno,from:frm,to,dep,arr:arr||null,overnight:!arr};
      cur.type="FLIGHT";
    }else{
      ...
      if(times.length&&!ex.arr){ex.arr=times[0];ex.overnight=false;}
    }
  }
  if(cur)days.push(cur);
...
const toICAO = fno => fno.replace(/^JQ/,"JST");
...
export default function CrewMate(){
  ...
  const [crew,     setCrew]   =useState({crewName:"",empId:"",
    totalBLH:0,flightBLH:0,role:"CM",
...
function TrackSheet({flight,onClose}){
  ...
  const fr=flight?.from||"";
  const to=flight?.to||"";
  // URL schemes for each tracking app
  const apps=[
    {
      name:"Flightradar24",
      icon:"📡",
      desc:"Best real-time tracking in AU/NZ",
      color:"#FF6B00",
      // FR24 deep link to specific flight
      url:`https://www.flightradar24.com/${fno}`,
      appUrl:`flightradar24://`,
      appStore:"https://apps.apple.com/au/app/flightradar24/id382233851",
    },
    {
      name:"Flighty",
      icon:"✈️",
      desc:"iOS-native, beautiful animations",
      color:"#0A84FF",
      url:`https://flighty.app/flights/${fno}`,
      appUrl:`flighty://flights/${fno}`,
      appStore:"https://apps.apple.com/au/app/flighty/id1358823008",
    },
    {
      name:"FlightAware",
      icon:"🛰",
      desc:"Detailed data, used by airports",
      color:"#2ECC71",
      url:`https://www.flightaware.com/live/flight/${fno}`,
      appUrl:`flightaware://`,
      appStore:"https://apps.apple.com/au/app/flightaware/id316793974",
    },
    {
      name:"OAG Flights",
      icon:"🗺",
      desc:"Popular in NZ & AUS aviation",
      color:"#A78BFA",
      url:`https://www.oag.com/flight-status/${fno}`,
      appUrl:null,
      appStore:null,
    },
  ];
  ...
  const openApp=(app)=>{
    // Try app URL scheme first, fall back to web
    if(app.appUrl){
    ...
    setTimeout(()=>window.open(app.url,"_blank"),200);
  };

  return(
    <div style={{position:"fixed",inset:0,background:"#000000E8",
      backdropFilter:"blur(16px)",zIndex:200,display:"flex",
      flexDirection:"column",justifyContent:"flex-end"}}>
      <div className="cm-sheet" style={{background:C.card,borderRadius:"24px 24px 0 0",
        paddingBottom:48,maxHeight:"85vh",overflowY:"auto"}}>
        <div style={{display:"flex",justifyContent:"center",padding:"14px 0 4px"}}>
          <div style={{width:40,height:4,borderRadius:2,background:C.muted}}/>
        </div>
        <div style={{padding:"10px 24px 18px",borderBottom:`1px solid ${C.border}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              <div style={{fontSize:10,color:C.green,fontWeight:700,letterSpacing:2,marginBottom:4}}>FLIGHT TRACKING</div>
              <div style={{fontSize:24,fontWeight:900}}>{fno}</div>
              {fr&&to&&<div style={{fontSize:13,color:C.soft,marginTop:2}}>{fr} → {to}</div>}
            </div>
            <button onClick={onClose} style={{background:C.card2,border:`1px solid ${C.border}`,
              borderRadius:12,color:C.soft,fontSize:18,width:40,height:40,cursor:"pointer",
              display:"flex",alignItems:"center",justifyContent:"center"}}>✕</button>
        ...
        <div style={{padding:"20px 24px 0"}}>
          <div style={{fontSize:12,color:C.soft,marginBottom:20,lineHeight:1.6}}>
            Open in your preferred tracking app. These are independent apps — CrewMate is not affiliated with any of them.
          </div>
          {apps.map(app=>(
            <button key={app.name} onClick={()=>openApp(app)}
              ...
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:11,color:C.soft,marginTop:2}}>{app.desc}</div>
              </div>
              <div style={{fontSize:20,color:C.muted}}>›</div>
            </button>
          ))}
          <div style={{marginTop:16,background:`${C.blue}10`,border:`1px solid ${C.blue}25`,
            borderRadius:14,padding:"12px 16px",display:"flex",gap:10}}>
            <span style={{fontSize:16}}>💡</span>
            <div style={{fontSize:12,color:C.soft,lineHeight:1.6}}>
              <strong style={{color:C.text}}>In-app map coming soon.</strong> A future version of CrewMate will show a live flight map directly — no app-switching needed.
            </div>
          </div>
        </div>
      </div>
...
function TodayTab({todayItems,tomorrowItems,crewInfo,onCrewTap}){
  ...
  return(
    <div>
      {regoSheet&&<RegoSheet profile={AIRCRAFT_PROFILE} liveRego={regoSheet} onClose={()=>setRegoSheet(false)}/>}
        ...
        return(
          <div key={i}>
            <FlightLiveCard
              ...
              onTrackTap={(fno,fr,to)=>setTrackSheet({flightNo:fno,from:fr,to})}
      ...
      {lastSignOff&&hasSignedOn&&(
        <div style={{background:`${C.blue}15`,border:`1px solid ${C.blue}40`,
          ...
          <SignOffTimer target={lastSignOff} hasSignedOn={hasSignedOn}/>
        </div>
      )}
    </div>
  );
...
function SignOffTimer({target,hasSignedOn}){
  ...
  useEffect(()=>{
    if(!hasSignedOn){setDiff(null);return;}
    const calc=()=>{
      const now=new Date();
...
function TimeUntil({target}){
  const [diff,setDiff]=useState(null); // null=loading, negative=past, positive=future mins
  useEffect(()=>{
    const calc=()=>{
      const now=new Date();
      const[h,m]=target.split(":").map(Number);
...
function FlightLiveCard({item,live,expanded,onToggle,onRegoTap,onTrackTap,onCrewTap,loading,isCurrent}){
    ...
    <div style={{marginBottom:10}}>
      <div onClick={onToggle} style={{background:C.card,
        ...
        <div style={{marginLeft:10}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginTop:4}}>
            {(live?.rego||item.rego)?(<button onPointerDown={e=>e.stopPropagation()}
              onClick={e=>{e.stopPropagation();e.preventDefault();onRegoTap&&onRegoTap(live?.rego||item.rego);}}
            ...
            </button>):(<button onPointerDown={e=>e.stopPropagation()}
              onClick={e=>{e.stopPropagation();e.preventDefault();onRegoTap&&onRegoTap(item.flightNo);}}
              style={{display:"inline-flex",alignItems:"center",gap:6,
                background:`${C.blue}10`,border:`1px dashed ${C.blue}40`,
                borderRadius:8,padding:"5px 11px",cursor:"pointer",
                WebkitTapHighlightColor:"transparent",touchAction:"manipulation",outline:"none"}}>
              <span style={{fontSize:11,color:C.blue}}>✈ Live Aircraft</span>
            </button>)}
            <button onPointerDown={e=>e.stopPropagation()}
      ...
      {expanded&&(
        <div style={{background:C.card2,borderRadius:"0 0 16px 16px",
          ...
          {loading&&!live?<div style={{textAlign:"center",padding:"12px 0",color:C.muted,fontSize:13}}>Fetching live data…</div>:(
            <div>
              ...
              <div style={{marginTop:10,display:"flex",gap:8,flexWrap:"wrap"}}>
                <a href={`https://www.flightradar24.com/${toICAO(item.flightNo)}`}
                  target="_blank" rel="noopener noreferrer"
                  style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",
                    gap:6,background:`${C.orange||"#FF8C00"}15`,border:`1px solid ${C.orange||"#FF8C00"}40`,
                    borderRadius:10,padding:"9px 10px",textDecoration:"none",
                    color:C.orange||"#FF8C00",fontSize:11,fontWeight:700}}>
                  🟠 Flightradar24
                </a>
                <a href={`https://flightaware.com/live/flight/${toICAO(item.flightNo)}`}
                  target="_blank" rel="noopener noreferrer"
                  style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",
                    gap:6,background:`${C.blue}15`,border:`1px solid ${C.blue}40`,
                    borderRadius:10,padding:"9px 10px",textDecoration:"none",
                ...
                <a href={`https://www.flightradar24.com/${toICAO(item.flightNo)}`}
                  target="_blank" rel="noopener noreferrer"
                  style={{flex:1,display:"flex",alignItems:"center",justifyContent:"center",
                    gap:6,background:`${C.purple}15`,border:`1px solid ${C.purple}40`,
                    borderRadius:10,padding:"9px 10px",textDecoration:"none",
              ...
              <div style={{marginTop:8,fontSize:10,color:C.muted,textAlign:"center"}}>
                Live data via ADS-B · Auto-refreshes every 10 min
              </div>
            </div>
          )}
        </div>
...
// ─── Aircraft rego sheet ──────────────────────────────────────────
function RegoSheet({profile,onClose,liveRego}){
  const [liveData,setLiveData]=useState(null);
  const [liveLoading,setLiveLoading]=useState(true);
  useEffect(()=>{
    const rego=liveRego||profile?.rego;
    if(!rego){setLiveLoading(false);return;}
    fetchAircraftByRego(rego).then(d=>{setLiveData(d);setLiveLoading(false);});
  },[liveRego,profile?.rego]);
    ...
    <div>
      ...
      {Object.keys(allDays).length===0?(
        ...
        <div key={date} style={{marginBottom:14}}>
          <div style={{fontSize:10,color:C.accent,letterSpacing:2,marginBottom:8}}>{date.toUpperCase()}</div>
          {entry.type==="duty"
