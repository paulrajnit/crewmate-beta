...
function TodayTab({todayItems,tomorrowItems,crewInfo,onCrewTap}){
  ...
  // Proactively fetch regos for today's flights on mount
  useEffect(()=>{
    ...
    (async()=>{
      ...
      for(const item of todayFlights){
        ...
        // Try callsign search
        try{
          const d=await fetchWithTimeout(`https://api.adsb.lol/v2/callsign/${icao}`,5000);
          if(d?.ac?.length>0&&d.ac[0].r){
            regos[item.flightNo]=d.ac[0].r;
            continue;
          }
        }catch(e){}
        // Try hex search via flight number
        try{
          const d2=await fetchWithTimeout(`https://api.adsb.lol/v2/flight/${icao}`,4000);
          if(d2?.ac?.length>0&&d2.ac[0].r)regos[item.flightNo]=d2.ac[0].r;
        }catch(e){}
      }
      if(Object.keys(regos).length>0)setRegoMap(rm=>({...rm,...regos}));
    })();
  },[todayFlights.length]);
...
function FlightLiveCard({item,live,itemRego,expanded,onToggle,onRegoTap,onTrackTap,onCrewTap,loading,isCurrent}){
            ...
            {(()=>{
              ...
              return(<button onPointerDown={e=>e.stopPropagation()}
                ...
                style={{display:"inline-flex",alignItems:"center",gap:6,
                  background:rego?`${C.blue}15`:`${C.blue}08`,
                  border:rego?`1px solid ${C.blue}40`:`1px dashed ${C.blue}30`,
                  borderRadius:8,padding:"5px 11px",cursor:"pointer",
                  WebkitTapHighlightColor:"transparent",touchAction:"manipulation",outline:"none"}}>
                <span style={{fontSize:12,fontWeight:700,color:rego?"#FFFFFF":C.blue,letterSpacing:.5}}>
                  {rego||"REGO"}
                </span>
                {rego&&<span style={{fontSize:11,color:C.blue}}>ⓘ</span>}
                {!rego&&<span style={{fontSize:10,color:C.blue}}>⟳</span>}
              </button>);
            })()}
            <button onPointerDown={e=>e.stopPropagation()}
