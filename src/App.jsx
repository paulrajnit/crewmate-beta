  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      for(const item of todayFlights){
        ...
        const strategies=[
          `https://api.adsb.lol/v2/callsign/${icao}`,          // live now
          `https://api.adsb.lol/v2/callsign/${item.flightNo}`,  // try IATA direct
        ];
        for(const url of strategies){
          try{
            const d=await fetchWithTimeout(url,6000);
            const ac=d?.ac?.[0]||d?.aircraft?.[0];
            if(ac?.r){regos[item.flightNo]=ac.r;break;}
            if(ac?.registration){regos[item.flightNo]=ac.registration;break;}
          }catch(e){}
        }
      }
        ...
        <div style={{padding:"10px 24px 18px",borderBottom:`1px solid ${C.border}`}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div>
              ...
              <div style={{fontSize:13,color:C.soft,marginTop:2}}>{profile.type}{profile.operator?" · "+profile.operator:""}</div>
