...
function TrackSheet({flight,onClose}){
  ...
  // Use rego for tracking when available (more reliable than callsign)
  // FR24: /data/aircraft/VH-VQI OR /{callsign} for live
  // FlightAware: /live/flight/{rego} OR /live/flight/{callsign}
  const fr24=rego
    ?`https://www.flightradar24.com/data/aircraft/${rego.toLowerCase()}`
    :`https://www.flightradar24.com/${icao}`;
  const faUrl=rego
    ?`https://www.flightaware.com/live/flight/${rego}`
    :`https://www.flightaware.com/live/flight/${icao}`;
  const planefUrl=rego
    ?`https://planefinder.net/data/aircraft/${rego}`
    :`https://planefinder.net/flight/${icao}`;

  const apps=[
    {
      name:"Flightradar24",
      icon:"📡",
      desc:rego?`Track ${rego} · best coverage AU/NZ`:"Live tracking · AU/NZ coverage",
      color:"#FF8C00",
      url:fr24,
    },
    {
      name:"FlightAware",
      icon:"🛰",
      desc:rego?`${rego} history & live position`:"Detailed flight data",
      color:"#2ECC71",
      url:faUrl,
    },
    {
      name:"Plane Finder",
      icon:"✈️",
      desc:rego?`Track ${rego} live`:"Real-time ADS-B tracking",
      color:"#A78BFA",
      url:planefUrl,
    },
  ];
